...
func convertNewSeverityToOld(newSeverity syslog.Severity) Severity {
	switch newSeverity {
	case syslog.Debug:
...
func convertNewMessageToOld(newMssg *syslog.Message) (*Message, error) {
	severity := convertNewSeverityToOld(newMssg.Severity)
	text := newMssg.String()
	application := piazza.ServiceName(newMssg.Application)
	hostname := newMssg.HostName
	tim := newMssg.TimeStamp
