...
func (m *SyslogMessage) String() string {
	...
	s := fmt.Sprintf("<%d>%d %s %s %s %d %s%s%s%s", (m.Facility*8 + m.Severity), m.Version, m.TimeStamp, host, m.Application, m.Process, m.MessageID, audit, metric, message)

	return s
}

func (ae *AuditElement) String() string {
	return fmt.Sprintf(`[pzaudit@%v actor="%s" action="%s" actee="%s"]`, "TODO", ae.Actor, ae.Action, ae.Actee)
}

func (me *MetricElement) String() string {
	return fmt.Sprintf(`[pzmetric@%v name="%s" value="%f" object="%s"]`, "TODO", me.Name, me.Value, me.Object)
}
